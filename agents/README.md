...
Edit each tracked file to change that tool's preferences. Both prefer TypeScript
and Node when feasible. Claude also requires the `unslop` skill for all writing.
Restore [skills](../skills/README.md) to make that skill available in Claude Code.

These are the default user-level locations documented by
...
Use the same procedure for `.claude/CLAUDE.md`. Newly created links have no backup;
